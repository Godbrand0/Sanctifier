    ...
    match FileWriter::write_config(&config, &target_dir) {
        Ok(config_path) => {
            ...
            if !lib_rs_path.exists() {
                fs::write(&lib_rs_path, "// Generated by sanctifier init\n")?;
            }

            let gitignore_path = target_dir.join(".gitignore");
            if gitignore_path.exists() {
                if let Ok(content) = fs::read_to_string(&gitignore_path) {
                    if !content.contains(".sanctifier_cache.json") {
                        let mut new_content = content;
                        if !new_content.ends_with('\n') {
                            new_content.push('\n');
                        }
                        new_content.push_str(".sanctifier_cache.json\n");
                        let _ = fs::write(&gitignore_path, new_content);
                    }
                }
            } else {
                let _ = fs::write(&gitignore_path, "target/\n.sanctifier_cache.json\n");
            }
